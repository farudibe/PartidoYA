...
export default function Register() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    setError(null)
    const { error, needsConfirmation } = await signUp(email, password, nombre, role)
    setLoading(false)
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-sm space-y-4 rounded-xl bg-white p-8 text-center shadow">
          <h1 className="text-2xl font-bold">Confirmá tu email ✉️</h1>
          <p className="text-sm text-gray-600">
            Te mandamos un mail a <span className="font-semibold">{email}</span> para confirmar tu cuenta.
            Abrí el link que te llegó y después volvé a iniciar sesión.
          </p>
